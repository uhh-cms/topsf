...
setup_topsf() {
    ...
    #   - "TOPSF_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "TOPSF_" variables is documented below.
    #
    ...
    if [ "${TOPSF_SETUP}" = "1" ]; then
        >&2 echo "the topsf analysis was already succesfully setup"
    ...
    #
    # global variables
    # (TOPSF = topsf, CF = columnflow)
    #

    # start exporting variables
    export TOPSF_BASE="${this_dir}"
    export CF_BASE="${this_dir}/modules/columnflow"
    export CF_REPO_BASE="${TOPSF_BASE}"
    ...
    if [ "${CF_REMOTE_JOB}" != "1" ]; then
        cf_setup_interactive_body() {
            ...
            query CF_DATA "Local data directory" "\$TOPSF_BASE/data" "./data"
            query CF_STORE_NAME "Relative path used in store paths (see next queries)" "topsf_store"
            ...
            query TOPSF_BUNDLE_CMSSW "Install and bundle CMSSW sandboxes for job submission?" "True"
        }
        cf_setup_interactive "${CF_SETUP_NAME}" "${TOPSF_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export CF_COMBINE_BASE="${CF_COMBINE_BASE:-${CF_SOFTWARE_BASE}/combine}"
    ...
    export PATH="${TOPSF_BASE}/bin:${PATH}"
    export PYTHONPATH="${TOPSF_BASE}:${TOPSF_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${TOPSF_BASE}/.git" ]; then
        local m
...
main() {
    ...
    if setup_topsf "$@"; then
        cf_color green "topsf analysis successfully set up"
        return "0"
    else
        local code="$?"
